...
class Page_change_text():
	def __init__(self):
		self.pageName = 'change_text'
		self.buttons = []
	...
	def Keyboard_Events(self, app_from, user_id, data):
		...
		sqlighter.set_data(table_name = 'users', line = 'text_anket', line_selector = f'user_id_{app_from}', line_selector_value = user_id, data = data)
		if ((data_page['data_page'] != None) and ('local_change' in data_page['data_page'])):
			return ImportFromStandart(page = 'settings', answers = ['Текст анкеты сохранён'])
		else:
